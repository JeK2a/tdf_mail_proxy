...
class PHPMailer
{
    ...
    /**
     * The Sender email (Return-Path) of the message.Could not acce
    ...
    public function useProxy($proxy_params = [])
    {
        $smtp_params = [];

        $smtp_params['host']     = $this->Host;           // host
        $smtp_params['port']     = $this->Port;           // port
        $smtp_params['user']     = $this->Username;       // login TODO ? =
        $smtp_params['from']     = $this->From;           // from  TODO ? =
        $smtp_params['password'] = $this->Password;       // password
        $smtp_params['char_set'] = $this->CharSet;        // charset
        $smtp_params['secure']   = $this->SMTPSecure;     // secure
        $smtp_params['to']       = array_keys($this->all_recipients); // [] to|cc|bcc
        $smtp_params['ReplyTo']  = array_keys($this->ReplyTo);        // [] |Reply-To
        $smtp_params['timeout']  = $this->Timeout;

        $this->proxy_smtp = new ProxySmtp($smtp_params, $proxy_params);

//        $this->setFrom($smtp_config['email'], $smtp_config['name_from']); // TODO ???
...
//        $this->SMTPAuth = true;
    ...
    public function getSMTPInstance()
    {
        if (!is_object($this->smtp)) {
            $this->smtp = new SMTP_PHPMailer($this->proxy_smtp);
        }
        return $this->smtp;
    }
...
//        $result .= 'EHLO tdf-service-1.online'."\r\n";
...
//            echo '~222~'.$path."-".$cid."\n";
    ...
    public function copyToFolder($folderPath = '&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-') // '&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-' - отправленные
    {
        $message = $this->MIMEHeader . $this->MIMEBody;
        $path = $folderPath;

        $imapStream = imap_open("{imap.yandex.ru:993/imap/ssl}" . $path , $this->Username, $this->Password);
        imap_append($imapStream, "{imap.yandex.ru:993}" . $path, $message);
        imap_close($imapStream);
    }

    public function copyToAccountFolder($account, $folderPath = '&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-') // '&BB4EQgQ,BEAEMAQyBDsENQQ9BD0ESwQ1-' - отправленные
    {
        $message = $this->MIMEHeader . $this->MIMEBody;
        $path = $folderPath;

        $imapStream = imap_open("{imap.yandex.ru:993/imap/ssl}" . $path , $account['login'], $account['password']);
        imap_append($imapStream, "{imap.yandex.ru:993}" . $path, $message);
        imap_close($imapStream);
    }
}
